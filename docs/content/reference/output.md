...
describes what users get. Pick a format with `-o`, or let chotot choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
chotot <command> -o table   # aligned columns for reading
chotot <command> -o jsonl   # one JSON object per line, for piping
chotot <command> -o json    # a single JSON array
chotot <command> -o csv     # spreadsheet friendly
chotot <command> -o tsv     # tab-separated
chotot <command> -o url     # just the URL column
chotot <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
chotot <command> --fields id,title,url
```
...
```bash
chotot <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
chotot <command>            # a table, because this is a terminal
chotot <command> | wc -l    # JSONL, because this is a pipe
```
